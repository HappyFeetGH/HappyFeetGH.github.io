...
물론, 만약 하나의 prototype을 모든 data vector에 할당한다면 zero distortion을 달성할 수 있다. 
하지만 이는 $$O(NDC)$$ 공간이 필요하다. 이때 $$N$$은 real-valued data vectors의 수이다. 각각의 길이는 $$D$$ 
$$C$$는 real-valued scalar를 표현하기 위해 필요한 bits의 수이다.   
하지만, 대부분의 data sets에서 비슷한 vectors를 반복적으로 확인 가능하다. 따라서 이들을 여러번 저장하는 것보다, 
한번만 저장하고, 그들에 대한 pointers를 생성하는 것이 좋다.   
+ 이때 공간을 $$O(N \log_2 K + KDC)$$만큼 줄일 수 있다.
    + $$O(N \log_2 K) 항은 $$N$$ data vectors의 각각이 그것이 사용된 $$K$$ codewords가 무엇인지 식별하기 위해 필요하다.
    + $$O(KDC)$$ 항은 각 codebook entry를 저장하기 위해 필요하다
    + 첫 번째 항은 두번째 항을 지배한다. 따라서 encdoing scheme의 **rate**의 근사치를 $$O(\log_2 K)$$로 구할 수 있다.

VQ의 사용처 예시로는 image compression이 있다.


**Initialization and avoiding local minima**
+ **farthest point clustering(k-means++)**:
    + K-means와 EM은 초기화해야 할 필요가 있다. $$K$$ data points를 무작위하게 뽑는것이 기본이다. 
그리고 이들이 initial cluster centers가 되도록 한다.   
    + 또는 centers를 순차적으로 뽑고, data를 "cover"하도록 시도한다. 이는 initial point를 무작위로 정규적으로 뽑는 것이다.   
    + 이후, 각 sbusequent point를 그것의 point의 가장 가까운 cluster center에
대한 제곱 거리에 대한 확률 비율로 남아있는 points로부터 뽑는다.

+ speech recognition community에서 사용되는 휴리스틱은 점진적으로 GMMs을 "grow"하는 것이다:
    + 각 군집에 score를 mixture weight에 기초하여 제공한다.
    + training 한 라운드가 끝나면, 가장 높은 점수를 가진 군집을 원본 중심의 무작위로 미동하는 새로운 둘의 중심들로 나눈다.
    + 새로운 점수는 원본의 반절이 된다
    + 새로운 군집이 너무 낮은 점수를 갖거나 분산이 작으면 제거된다
    + 위를 적당한 수의 군집이 나올때까지 반복한다


**MAP estimation**
+ MLE가 overfit한경우 - 특히 GMMs에서 자주 보인다.
+ 예시) $$\Sigma_k = \sigma_{k}^{2}I$$, $$K=2$$라 가정한다
+ 중심들 중에 하나를 single data point $$x_1$$에할당함으로써 infinite likelihood를 
갖는것이 가능하다. 이를 $$\mu2$$라부른다.
    + 왜냐하면 첫번째 항이 likelihood에 대한 다음의 영향을 주기 때문이다.
	+ $$N(x_1 | \mu_2, \simga_{2}^{2}I) = \frac{1}{\root{2\pi\sigma_{2}^{2}} e^0$$
