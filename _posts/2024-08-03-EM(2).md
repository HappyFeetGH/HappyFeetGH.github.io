...
$$ = \sum_i E[\log[ \pod_{k=1}^{K} (\pie_k p (x_i | \theta_k))^{\parallel (z_i = k)}]] $$   
$$ = \sum_i \sum_k E[\parallel (z_i = k)] \log[\pie_k p(x_i | theta_k)]$$   
$$ = \sum_i \sum_k p(z_i = k | x_i, \theta^{t-1}) \log[\pie_k p(x_i | \theta_k)] $$   
$$ = \sum_i \sum_k r_{ik} \log \pie_k + \sum_i \sum_k r_{ik} \log p(x_i | \theta_k) $$   
...
$$r_{ik} = \frac{\pie_k p(x_i | \theta_k^{t-1}}{\sum_{k'} \pie_{k'} p(x_i | \theta_{k'}^{(t-1}}}$$   


**M step**   
M step에서, $$\pie$$와 $$\theta_k$$에 대하여 $$Q$$를 최적화한다. $$\pie$$에 대해서 다음과 같다:   
$$\pie_k = \frac{1}{N} \sum_i r_{ik} = \frac{r_k}{N} $$   
